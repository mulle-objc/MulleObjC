...
### 4.3. Forward Method and Property Declarations

-   Do not use forwarding categories to declare forward methods or properties.
-   Instead, use `@optional` within the `@interface` itself.
    ...
    @optional
    - (void) forwardMethod;
    @property( forward, assign) int forwardProperty;

    @end
    ```

### 4.4 Employ Factory style object creation

-   prefer [Foo instance] instead of [[Foo new] autorelease] or [[[Foo alloc] init] autorelease]
-   prefer multiple line instantation with +instantiate instead of [[[Foo alloc] init] autorelease].
    This is easier to debug and avoids -autorelease in user code
