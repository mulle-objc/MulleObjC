...
static void   test_valid_in_range( NSRange range1, NSRange range2)
{
   printf( "[%ld - %ld] is valid in range [%ld - %ld] ? %s\n",
               (long) range1.location, (long) (range1.location + range1.length),
               (long) range2.location, (long) (range2.location + range2.length),
      MulleObjCRangeIsValidInRange( range1, range2) ? "YES" : "NO");
         ...
         for( range2.location = -2; range2.location != 3; range2.location++)
         {
            for( range2.length = -2; range2.length != 3; range2.length++)
               if( MulleObjCRangeIsValid( range2))
                  test_valid_in_range( range1, range2);
         }
         printf( "\n");
      }
   return( 0);
